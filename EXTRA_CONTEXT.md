...
- [x] **T0.1**: Create monorepo structure and Python package skeleton (`src/pf`).
- [x] **T0.2**: Add `pyproject.toml` with Ruff/Pyright/Pytest config.
- [x] **T0.3**: Add CI workflows (Python checks + LaTeX build with BibTeX).
- [x] **T0.4**: Add `CONTRIBUTING.md` and tests philosophy docs.
- [x] **T0.5**: Add LaTeX paper skeleton and bibliography.

Depends: none

### Epic 1 — Data pipeline v0 (Stooq only)
- [x] **T1.1**: Canonical schemas and parquet store utilities (`src/pf/data/schemas.py`, `store.py`).
- [x] **T1.2**: Implement `StooqProvider` (prices only) in `src/pf/data/providers/stooq.py`.
- [ ] **T1.3**: Implement CLI command `pf data fetch` with config parsing.
- [x] **T1.4**: Integration tests:
  - [x] provider output conforms to schema
  - [~] parquet roundtrip and no data loss (implicitly tested via `fetch_prices` caching, but could add explicit test)
  - [~] dataset invariants: monotonic dates, finite returns, no duplicates (could add explicit invariant tests)
